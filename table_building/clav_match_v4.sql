...
SELECT
  ...
  a.match_score AS match_score FROM (
  SELECT
    clav_VRMFID,
    mmsi,
    rank_imo,
  ...
  FROM (
    ...
    FROM (
      ...
      FROM (      
        SELECT *, CASE WHEN (rank_imo = 0
          AND rank_callsign = 0
          AND flag_match = 0) THEN 0 
          
          WHEN (rank_imo = 0
          AND rank_callsign = 0
          AND flag_match =1
          AND shiptype_fishing = 1
          AND  fishing_msg_ratio is null) THEN 0 
          
          WHEN(rank_imo = 0
          AND rank_name = 0
          AND flag_match =1
          AND shiptype_fishing = 1
          AND fishing_msg_ratio is null) THEN 0 ELSE msg_score END as msg_score2
        FROM(
...
ON
  ...
  WHERE RIGHT(string(mmsi), 6) != '000000'